...
public class PackageService : IPackageService
{
    ...
    public async Task<Result> ReceivePackageAsync(int packageId, string serialNumber)
    {
        ...
        var bindingResult = await CheckLockerToClientBindingAsync(packageResult.Value.PostBoxId, serialNumber);
    ...
    public async Task<Result<bool>> CheckLockerToClientBindingAsync(int postBoxId, string serialNumber)
    {
        // Get user by NFC serial number
        var userIdResult = await _nfcService.GetUserIdByNfcSerialAsync(serialNumber);
        if (!userIdResult.IsSuccess || !userIdResult.Value.HasValue)
        ...
        var package = allPackagesResult.Value.FirstOrDefault(
            p => p.DeliveryStatus != null && p.DeliveryStatus.Name == "Delivered");
    ...
    public async Task<Result<IEnumerable<int>>> OpenAllDeliveredLockersForUserAsync(string serialNumber)
    {
        // Get user by NFC serial number
        var userIdResult = await _nfcService.GetUserIdByNfcSerialAsync(serialNumber);
        if (!userIdResult.IsSuccess || !userIdResult.Value.HasValue)
        {
            return Result<IEnumerable<int>>.Failure(
        ...
        if (!allPackagesResult.IsSuccess)
        {
            return Result<IEnumerable<int>>.Failure(allPackagesResult.Errors);
        }

        // Filter packages with Delivered status and get unique PostBoxIds
        var deliveredPackages = allPackagesResult.Value
            .Where(p => p.DeliveryStatus != null && p.DeliveryStatus.Name == "Delivered" && p.PostBoxId > 0)
            .ToList();

        var postBoxIds = deliveredPackages
            .Select(p => p.PostBoxId)
            .Distinct()
            .ToList();
        ...
        _logger.LogInformation(
            "Opening {Count} lockers for user {UserId} with NFC {SerialNumber}: {PostBoxIds}",
            postBoxIds.Count, userId, serialNumber, string.Join(", ", postBoxIds));

        // In a real system, this would communicate with IoT lockers to open them
        // For now, just return the list of PostBoxIds that would be opened
        return Result<IEnumerable<int>>.Success(postBoxIds);
    }
}