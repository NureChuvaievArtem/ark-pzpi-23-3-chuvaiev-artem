...
try:
    ...
    lcd = I2cLcd(i2c, 0x27, 2, 16)  # I2C адрес 0x27, 2 ряда, 16 символов
    LCD_AVAILABLE = True
...
def lcd_input(prompt, max_length=10, numeric_only=False):
    input_buffer = ""
...
# API Configuration
# ==========================================
API_BASE_URL = "https://packagedeliverybackendwindows-hvg6f7brdhfnfhdn.polandcentral-01.azurewebsites.net"

# ==========================================
# Hardware Configuration
# ==========================================
LED_SUCCESS = Pin(2, Pin.OUT)  
LED_ERROR = Pin(4, Pin.OUT)   
RELAY = Pin(15, Pin.OUT)      

# ==========================================
# ЛОКАЛЬНА БАЗА ДАНИХ КОМІРОК
# ==========================================
LOCKER_DATABASE = [
    {
        'id': 1,
...
def open_locker(locker_number, duration=5):
...
def calculate_optimal_locker(package_height, package_width, package_depth, available_lockers=None):
    ...
    for locker in sorted_lockers:
        ...
        if available_space >= package_volume:
            ...
            if 60 <= utilization_percent <= 85:
                efficiency_score = 100.0
            elif utilization_percent < 60:
                efficiency_score = 70.0 + (utilization_percent / 60.0) * 30.0
            else:
                penalty = ((utilization_percent - 85.0) / 15.0) * 30.0
    ...
    def handle_courier_mode(self):
        ...
        for idx, package in enumerate(packages):
            ...
            if optimal:
                ...
                # Чекаємо підтвердження з timeout (30 секунд)
                key = None
